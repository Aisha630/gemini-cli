...
function getHighlightedReverseSearchText(
  ...
  const matchIndex = bufferText
    .toLowerCase()
    .indexOf(reverseSearchQuery.toLowerCase());

  if (matchIndex === -1) return chalk.dim(display);
          ...
          {buffer.text.length === 0 && placeholder ? (
            ...
            linesToRender.map((lineText, visualIdxInRenderedSet) => {
              ...
              if (reverseSearchActive && shellModeActive) {
                display =
                  buffer.text && reverseSearchQuery
                    ? getHighlightedReverseSearchText(
                        display,
                        buffer.text,
                        reverseSearchQuery,
                      )
                    : reverseSearchQuery
                      ? chalk.hex(Colors.AccentGreen)(reverseSearchQuery)
                      : '';
              }
